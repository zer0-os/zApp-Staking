...
test('must approve token spending before staking', async ({ page }) => {
	...
	await expect(page.getByText('Approve Pool Spending')).toBeVisible();
});

test('can approve token spending', async ({ page }) => {
